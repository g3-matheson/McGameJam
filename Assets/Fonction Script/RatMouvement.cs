...
public class RatMouvement : MonoBehaviour
{
    ...
    void FleeBehaviour()
    {
        if (nearestDeathPoint == null && SafeZoneManager.Instance != null)
        {
            nearestDeathPoint =
                SafeZoneManager.Instance.GetNearestDeathPoint(transform.position);
        }

        Vector2 direction;

        if (nearestDeathPoint != null)
        {
            direction = ((Vector2)nearestDeathPoint.position -
